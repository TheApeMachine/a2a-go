...
type TaskManager interface {
    SendTask(ctx context.Context, params TaskSendParams) (Task, *rpcError)
    GetTask(ctx context.Context, id string, historyLength int) (Task, *rpcError)
    CancelTask(ctx context.Context, id string) (Task, *rpcError)
    ...
    // stream is finished.
    StreamTask(ctx context.Context, params TaskSendParams) (<-chan interface{}, *rpcError)
}
...
func NewEchoTaskManager(store *InMemoryTaskStore) *EchoTaskManager {
    if store == nil {
...
func (m *EchoTaskManager) SendTask(ctx context.Context, p TaskSendParams) (Task, *rpcError) {
    // extract first text part
    txt := ""
    if len(p.Message.Parts) > 0 && p.Message.Parts[0].Type == PartTypeText {
    ...
    task := Task{
        ...
        Artifacts: []Artifact{{
            Parts: []Part{{Type: PartTypeText, Text: txt}},
            Index: 0,
        }},
    }
...
func (m *EchoTaskManager) GetTask(ctx context.Context, id string, historyLength int) (Task, *rpcError) {
    // For the echo manager we don’t keep full tasks in store, so return a not
    // found error unless SendTask already produced it.
    e, ok := m.store.Get(id)
    if !ok {
        return Task{}, &rpcError{Code: -32001, Message: "Task not found"}
    }
    task := Task{
        ID: e.ID,
        Status: TaskStatus{
...
func (m *EchoTaskManager) CancelTask(ctx context.Context, id string) (Task, *rpcError) {
    ok := m.store.UpdateState(id, TaskStateCanceled)
    if !ok {
        return Task{}, &rpcError{Code: -32001, Message: "Task not found"}
    }
    task := Task{ID: id, Status: TaskStatus{State: TaskStateCanceled}}
    return task, nil
}
...
func (m *EchoTaskManager) StreamTask(ctx context.Context, p TaskSendParams) (<-chan interface{}, *rpcError) {
    ...
    go func() {
        ...
        // artifact
        ch <- TaskArtifactUpdateEvent{
            ID: p.ID,
            Artifact: Artifact{
                Parts: []Part{{Type: PartTypeText, Text: "streamed echo: " + p.Message.Parts[0].Text}},
                Index: 0,
            },
        }
    ...
    m.store.UpdateState(p.ID, TaskStateWorking)
