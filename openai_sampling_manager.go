...
func (o *OpenAISamplingManager) CreateMessage(ctx context.Context, content string, opts sampling.SamplingOptions) (*sampling.SamplingResult, error) {
    ...
    if content != "" {
        msgs = append([]Message{{Role: "user", Parts: []Part{{Type: PartTypeText, Text: content}}}}, msgs...)
    }
...
func convertSamplingContext(c *sampling.Context) []Message {
    if c == nil {
        return nil
    }
    out := make([]Message, len(c.Messages))
    for i, m := range c.Messages {
        out[i] = Message{Role: m.Role, Parts: []Part{{Type: PartTypeText, Text: m.Content}}}
    }
    return out
}
