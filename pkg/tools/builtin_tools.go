...
func RegisterBuiltInTools(srv *server.MCPServer, store *stores.InMemoryTaskStore) {
	if store == nil {
...
var defaultTaskStore = stores.NewInMemoryTaskStore()

// makeOrchestratorHandler returns the MCP tool handler closure capturing the
// task store instance.
func makeOrchestratorHandler(store *stores.InMemoryTaskStore) func(context.Context, mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		store.UpdateState(parentID, types.TaskStateWorking)
		...
		go func() {
			for i, sub := range subObjs {
				...
				store.UpdateState(childID, types.TaskStateWorking)

				// Simulate some processing work.
				time.Sleep(300 * time.Millisecond)
				store.UpdateState(childID, types.TaskStateCompleted)
			}

			// All children done → mark parent completed.
			store.UpdateState(parentID, types.TaskStateCompleted)
		}()
